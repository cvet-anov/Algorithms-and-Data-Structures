...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        ...
        average/=arr.getSize();
        float najmal=Math.abs(arr.get(0)/average);
        for (int i=1;i<arr.getSize();i++)
        {
            if (Math.abs(arr.get(i)-average)<najmal)
            {
                najmal=Math.abs(arr.get(i)-average);
                index=arr.get(i);
            }

            if (Math.abs(arr.get(i)-average)==najmal && arr.get(i)<index)
            {
                najmal=Math.abs(arr.get(i)-average);
                index=arr.get(i);
            }
